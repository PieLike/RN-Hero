...
{
    public enum CellType2 { alcPot, alcSlot, invSlot, armSlot, wpnSlot };
    public CellType2 cellType2;
    private InterfaceManager interfaceManager; Inventory inventory;
    ...
    public void OnDrop(PointerEventData data)
    {
        if (DragAndDropItem.icon != null)
        {
            ...
            {
                if ((item != null) && (sourceCell != this) && CanBeDrop(item))
                {
    ...
    private bool CanBeDrop(DragAndDropItem item)
    {
        ...
        if (itemData != null)
        {
            switch (cellType2)
            {
                case CellType2.wpnSlot:
                    if (itemData.data.itemType == ItemData.ItemType.weapon)
                        return true;
                    break;
                case CellType2.armSlot:
                    if (itemData.data.itemType == ItemData.ItemType.armor)
                        return true;
                    break;
                case CellType2.invSlot:
                    return true;
                default:
                    break;
            }
        ...
        {
            WordData wordData = item.GetComponent<WordData>();
            if (wordData != null)
            {
                return true;
            }
	...
	private void PlaceItem(DragAndDropItem item)
	{
        if (item != null)
		{
			...
			if (cell != null)
			{
                RectTransform iconRect = item.GetComponent<RectTransform>();

                iconRect.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, rectTransform.rect.height - 5f);
                iconRect.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, rectTransform.rect.width - 5f);
            ...
            if (cellType2 == CellType2.alcPot)
            {
                Alchemy alchemy = interfaceManager.Alchemy.GetComponent<Alchemy>();
                WordData wordData = item.gameObject.GetComponent<WordData>();
                if (wordData != null)
                {
                    alchemy.AddToProps(wordData.word);
                }  
                Destroy(item.gameObject);
                
            }
            else if (cellType2 == CellType2.alcSlot)
            {       
                AlcScroll alchemy = interfaceManager.Alchemy.GetComponent<AlcScroll>();
                WordData wordData = item.gameObject.GetComponent<WordData>();
                if (wordData != null)
                {
                    alchemy.AddScrollElement(wordData.word);
                }    
                if (item.gameObject != null)                     
                    Destroy(item.gameObject);
            }
            else if(cellType2 == CellType2.wpnSlot)
            {           
                item.transform.SetParent(transform, false);
                item.transform.localPosition = Vector3.zero;
                item.MakeRaycast(true);
                myDadItem = item;
                inventory.MeleeChange(item.gameObject);
            }
            else
            {
                item.transform.SetParent(transform, false);
    ...
    private void DestroyItem()
    {
		...
		if (myDadItem != null)
        {
            ...
            if(cellType2 == CellType2.wpnSlot)
                inventory.MeleeChange();
